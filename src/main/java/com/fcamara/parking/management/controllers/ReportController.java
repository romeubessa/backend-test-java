...
public class ReportController {
    ...
    @GetMapping("/summary/{establishmentId}")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved summary",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ParkingSummaryResponseDTO.class))),
    ...
    @GetMapping("/hourly-summary/{establishmentId}")
    ...
    public ResponseEntity<Object> getHourlySummary(@PathVariable String establishmentId) {
        var hourlySummary = reportService.getHourlySummary(establishmentId);

        return ResponseEntity.ok(hourlySummary);
    }

    @GetMapping("/vehicle-movements/{establishmentId}")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully fetched vehicle movement report",
                    content = {@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = VehicleMovementReportResponseDTO.class)))}),
            @ApiResponse(responseCode = "404", description = "Establishment not found",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseDTO.class))),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<List<VehicleMovementReportResponseDTO>> getVehicleMovementReport(
        ...
        return ResponseEntity.ok(vehicleMovements);
    }
}
